...
const {each, get, has, isEmpty, isNaN, isNumber} = require('lodash');
const minimist = require('minimist');
const path = require('path');

const basis = require('./lib/basis');
const config = require('./config');
const sumOfField = require('./lib/sum-of-field');
...
<html>
  <head id="head">
    ...
    <meta http-equiv="refresh" content="${0.001 *
      (get(info, 'nextTime', 0) - get(info, 'createdAt', 0))}">
    <title>CRYPTO!</title>
  </head>
  <body>
    <div class="item html">
  ...
  ).then(data => fse.writeFile('index.html', data));
/*
    ${buildDiv(data)}
    <pre>
  ...
  Promise.try(() => main(args))
    .tap(data =>
      Promise.resolve({
        ...
        !get(context, 'config.server.elasticsearch.disabled')
